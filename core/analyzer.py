...
COLOR_EMOTION_MAP = {
    "красный":       {"агрессия": 0.8, "энергия": 0.7, "радость": 0.3},
    "оранжевый":     {"радость": 0.9, "энергия": 0.7},
    "жёлтый":        {"радость": 1.0, "энергия": 0.6},
    "жёлто-зелёный": {"тревога": 0.6, "страх": 0.3},
    ...
    "розовый":       {"любовь": 1.0, "радость": 0.6},
}
...
ZONE_EMOTION_MAP = {
    "верх":  {"высокая": {"радость": 15, "энергия": 10},    "низкая": {"грусть": 15, "одиночество": 10}},
    "центр": {"высокая": {"энергия": 10, "агрессия": 5},     "низкая": {"страх": 15, "одиночество": 15}},
    "низ":   {"высокая": {"тревога": 20, "страх": 10},       "низкая": {"спокойствие": 10}},
    "лево":  {"высокая": {"грусть": 10, "одиночество": 5},   "низкая": {}},
    "право": {"высокая": {"радость": 10, "энергия": 5},      "низкая": {"тревога": 10, "страх": 5}},
}
...
LINE_PRESSURE_MAP = {
    "сильный": {"агрессия": 25, "тревога": 15, "энергия": 10},
    "средний": {"спокойствие": 10},
    "слабый":  {"грусть": 10, "одиночество": 8},
}

LINE_CHARACTER_MAP = {
    "прерывистые": {"тревога": 20, "страх": 15, "одиночество": 10},
    "плавные":     {"спокойствие": 15, "радость": 5},
    "угловатые":   {"агрессия": 15, "тревога": 10},
}
...
JOY_SIGNATURES = {
    ...
    "nature_scene": {
        ...
        "bonus": {"радость": 25, "спокойствие": 15, "энергия": 10},
...
VETO_RULES = [
    # Тёмная ахроматическая — радость и энергия заблокированы
    {
        ...
        "caps":   {"радость": 15, "энергия": 10},
        "boosts": {"грусть": 30, "страх": 20, "одиночество": 20},
        "label":  "Тёмная ахроматическая палитра",
    },
    # Тёмная палитра (любая)
    {
        "condition": lambda c, comp, z, l: c["brightnessValue"] < 35,
        "caps":   {"радость": 25, "энергия": 20},
        "boosts": {"грусть": 20, "тревога": 15},
    ...
    # Закрашенный лист одним цветом (объектов ≤ 2)
    {
        ...
        "caps":   {"энергия": 20, "радость": 20},
        "boosts": {"тревога": 25, "страх": 15},
    ...
    # Слабый нажим + тёмная холодная → подавленность
    {
        ...
        "caps":   {"агрессия": 15, "энергия": 20},
        "boosts": {"грусть": 15, "одиночество": 15},
    ...
    # Ахроматическая штриховка (хаос без цвета)
    {
        ...
        "caps":   {"радость": 20, "энергия": 15},
        "boosts": {"тревога": 20, "одиночество": 10},
    ...
    # Светлый холодный рисунок с малым покрытием → грусть
    {
        ...
        "boosts": {"грусть": 25, "одиночество": 20},
...
class DrawingAnalyzer:
    ...
    def _scores_luscher(self, color: dict, comp: dict, age) -> dict:
        ...
        bri_delta = {
            "тёмный":  {"страх": 35, "грусть": 30, "тревога": 25, "радость": -30, "энергия": -20},
            "средний": {},
            "светлый": {"радость": 20, "спокойствие": 15, "страх": -20, "грусть": -10},
        }
        ...
        # Насыщенность
        sat_delta = {
            "серый":     {"грусть": 30, "одиночество": 30, "спокойствие": -15, "радость": -20},
            "приглушён": {"грусть": 10, "спокойствие": 5},
            "яркий":     {"радость": 25, "агрессия": 10, "энергия": 20},
        }
        ...
        if cov > 30:
            sc["радость"] += 15; sc["энергия"] += 10
        elif cov > 15:
            sc["радость"] += 8;  sc["энергия"] += 5
        elif cov < 5:
            sc["грусть"] += 10;  sc["одиночество"] += 10

        # Природная палитра — бонус радости
        if color.get("palette") in ("природная", "многоцветная"):
            sc["радость"] += 20; sc["спокойствие"] += 10; sc["энергия"] += 10

        # Тёплая палитра — бонус радости
        if color.get("palette") == "тёплая":
            sc["радость"] += 10; sc["энергия"] += 5

        # Разнообразие цветов
        nv = color.get("nVividColors", 0)
        if nv >= 4:
            sc["радость"] += 15; sc["энергия"] += 8
        elif nv >= 2:
            sc["радость"] += 8
        ...
        fill = comp["fillClass"]
        if fill == "низкая":
            sc["одиночество"] += 15; sc["грусть"] += 10
        elif fill == "высокая" and comp["numObjects"] > 3:
            sc["энергия"] += 15; sc["радость"] += 10

        loc = comp["location"]
        if loc == "низ":   sc["тревога"] += 12; sc["страх"] += 6
    ...
    def _scores_makeover(self, zone: dict, age) -> dict:
        ...
        if vb < -10: sc["тревога"] += 15; sc["страх"] += 10
        elif vb > 10: sc["радость"] += 10; sc["энергия"] += 5
        if hb < -10: sc["грусть"] += 10; sc["одиночество"] += 10
        elif hb > 10: sc["энергия"] += 10; sc["радость"] += 5
        if age and age < 5:
    ...
    def _scores_lines(self, line: dict) -> dict:
        ...
        if line["thickness"] == "толстые":
            sc["агрессия"] += 10; sc["энергия"] += 5
        elif line["thickness"] == "тонкие":
            sc["грусть"] += 10; sc["страх"] += 5
        if line["chaos"] == "высокая":
            sc["тревога"] += 25; sc["страх"] += 10
    ...
    def _evidence(self, em: str, color: dict, comp: dict, zone: dict, line: dict) -> str:
        ...
        ev = {
            ...
            "страх":       f"Яркость {color['brightnessValue']}%; нажим: {line['pressure']}; центр: {zc.get('центр','—')}",
            "агрессия":    f"Нажим: {line['pressure']}; толщина: {line['thickness']}; центр: {zc.get('центр','—')}",
            "спокойствие": f"Линии: {line['character']}; палитра: {color['palette']}",
            "одиночество": f"Заполнение {comp['fillRatio']}%; нажим: {line['pressure']}; левая зона: {zc.get('лево','—')}",
            "любовь":      f"Палитра: {color['palette']}; тёплые тона: {color.get('warmRatio',0)}%",
            "энергия":     f"Нажим: {line['pressure']}; палитра: {color['palette']}; правая зона: {zc.get('право','—')}",
        }
    ...
    def _recommendations(self, emotions, comp, zone, line, age) -> list:
        ...
        if "тревога" in names or "страх" in names:
        ...
        if "одиночество" in names or "грусть" in names:
            recs.append("Уделить повышенное внимание общению, расширить круг социальных контактов")
    ...
    def _risk_factors(self, emotions, color, comp, zone, line) -> list:
        ...
        if em.get("страх", 0) > 50:      risks.append("Высокий уровень страха (>50%)")
        if em.get("агрессия", 0) > 60:   risks.append("Повышенная агрессивность (>60%)")
        if em.get("тревога", 0) > 55:    risks.append("Выраженная тревожность (>55%)")
        ...
        if em.get("одиночество", 0) > 50 and em.get("грусть", 0) > 40:
        ...
        if zc.get("лево") == "высокая" and zc.get("право") == "низкая":
            risks.append("Маховер: фиксация на прошлом, страх будущего")
        return risks
